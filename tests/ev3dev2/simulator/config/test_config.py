...
class TestConfig(unittest.TestCase):
    ...
    def test_load_word_config(self):
        ...
        self.assertEqual(args[:3], ('world_configurations', 'config_large', None))
        ...
        self.assertEqual(args[:3], ('world_configurations', 'config_small', None))
        ...
        self.assertEqual(len(conf._load_yaml_file.mock_calls), 1)
        fn_name, args, kwargs = conf._load_yaml_file.mock_calls[0]
        self.assertEqual(args[:3], ('world_configurations', 'local_config.yaml', 'test_path'))

    def test_load_yaml_file(self):
        conf = config.Config(None)
        val = conf._load_yaml_file('world_configurations', 'config_small').data
        ...
        with patch('ev3dev2simulator.config.config.Config.get_project_root') as ConfigMock:
            ...
            self.assertRaises(ScannerError, conf._load_yaml_file, 'world_configurations', 'config_small_error')
